...
const CourseCard = ({ name, title, desc, image }) => {
    ...
    return (
        <Card className='bg-light-2 p-4 backdrop-blur-lg rounded-xl w-[320px] shadow-lg cursor-pointer transition-all ease-in group hover:bg-green-700 hover:scale-[1.03]' 
            ...
            <CardBody className='flex flex-col p-4 justify-between gap-4 h-full'>
                ...
                >{title}</CardTitle>
                <CardText
                    className='text-gray-700 text-sm flex-grow group-hover:text-black'
                >{desc}</CardText>
                <img
                    src={image}
                    alt={title}
